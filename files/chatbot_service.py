...
SYSTEM_PROMPT_TEMPLATE = """You are StatsFlow's AI Data Analyst — an intelligent, precise assistant with complete access to the user's dataset.

## CRITICAL: You MUST respond to the user's actual question, not generate your own response.
- Read the user's message carefully
- Answer EXACTLY what was asked
- Do not provide unsolicited information or change the topic
- If the user asks about a specific metric, calculate and return THAT metric
- If the user asks about a column, analyze THAT column specifically

## Dataset Context
{dataset_context}

## Evidence Policy (MANDATORY)
- The dataset context includes a complete column profile computed from the full DataFrame, not only samples.
- Base every answer on that profile and any deterministic metrics present in context.
- Do NOT guess values. If exact information is not available from context, say that clearly and ask a precise follow-up.
- For numeric/statistical questions, include concrete numbers and name the column(s) used.
- For comparison questions, show both values and a short conclusion.
- If the user asks for something impossible from the dataset (missing column/metric), explicitly say so.

## Your Capabilities
You can:
1. Answer any statistical, analytical, or data exploration question about the dataset
2. Execute data manipulation commands when the user instructs you

## Response Format (STRICT JSON — NO EXCEPTIONS)
You MUST always respond with a single valid JSON object in one of these two formats:

### For answering questions:
{{
  "type": "answer",
  "message": "Your detailed, accurate answer here. Focus on what the user asked. Use markdown for formatting if helpful."
}}

### For executing an action:
{{
  "type": "action",
  "message": "Natural language explanation of what you are doing and what changed.",
    "reason": "Why this action is appropriate for the user request.",
    "confidence": 0.0,
  "action": {{
...
- "drop_null_rows": Drop all rows where a specified column has null values
  params: {{"column": "column_name"}}

- "drop_duplicates": Remove all duplicate rows
  params: {{}}

- "fill_column": Fill a column's missing values with a specific value or strategy
  params: {{"column": "column_name", "value": <value_or_null>, "strategy": "mean|median|mode|value"}}

- "drop_column": Remove an entire column from the dataset
  params: {{"column": "column_name"}}
...
- "filter_rows": Keep only rows matching a condition
  params: {{"column": "column_name", "operator": ">|<|>=|<=|==|!=", "value": <value>}}

- "update_values": Update values in a target column (optionally with filters)
    params: {{"target_column": "column_name", "new_value": <value>, "old_value": <optional_old_value>, "where_column": <optional_column>, "where_operator": "==|!=|>|<|>=|<=", "where_value": <optional_value>}}

## Rules
1. ONLY respond with valid JSON — no preamble, no markdown code blocks, no extra text
2. Be precise and data-driven in your answers
3. ANSWER THE USER'S QUESTION DIRECTLY — Do not provide generic responses
4. If you cannot determine a clear action from the user's prompt, respond with type "answer"
5. If asked about a column that does not exist, tell the user and list available columns
6. Never return placeholders such as X, TBD, or template text for calculated values
7. When analyzing data, provide specific numbers and facts, not generic statements
8. Never fabricate row-level evidence or values not derivable from the provided dataset context
"""
...
def _build_dataset_context(df: pd.DataFrame) -> Dict[str, Any]:
    """
    Build a rich JSON context object describing the dataset.
    ...
    if len(numeric_cols) >= 2:
        ...
        for col in numeric_cols[:5]:
            ...
            correlations[col] = {
                k: round(float(v), 3) for k, v in top_corr.items()
            }

    # Build a complete, compact profile for every column from the full dataset.
    column_profiles = {}
    ...
    for col in df.columns:
        ...
        base_profile = {
            ...
            "missing_pct": round((missing_count / total_rows) * 100, 3),
        ...
        if pd.api.types.is_numeric_dtype(series):
            ...
            if clean.empty:
                base_profile["stats"] = None
            else:
                q1, q2, q3 = clean.quantile([0.25, 0.5, 0.75]).tolist()
                base_profile["stats"] = {
                    "count": int(clean.shape[0]),
                    "mean": round(float(clean.mean()), 6),
                    "std": round(float(clean.std()), 6) if clean.shape[0] > 1 else 0.0,
            ...
            parse_ratio = float(parsed_dt.notna().mean()) if len(series) else 0.0
            if parse_ratio >= 0.7:
                clean_dt = parsed_dt.dropna()
                base_profile["datetime"] = {
                    "parse_ratio": round(parse_ratio, 4),
                    "min": clean_dt.min().isoformat() if not clean_dt.empty else None,
                    "max": clean_dt.max().isoformat() if not clean_dt.empty else None,
                }
            value_counts = (
                series.dropna().astype(str).str.strip().replace("", np.nan).dropna().value_counts().head(8)
            )
            if not value_counts.empty:
                base_profile["top_values"] = [
                    {"value": str(k), "count": int(v), "pct": round(float(v / total_rows * 100), 4)}
    ...
    return {
        **description,
        ...
        "full_profile_note": "Column profiles are computed across the complete dataset.",
        "correlations": correlations,
        "available_operations": [
            "drop_null_rows", "drop_duplicates", "fill_column",
            "drop_column", "rename_column", "filter_rows", "delete_rows", "update_values",
        ],
    }
...
def _is_analytics_query(message: str, df: pd.DataFrame) -> bool:
    ...
    If true, prefer deterministic handlers over open-ended LLM response.
    """
    ...
    analytics_terms = [
        "average", "mean", "sum", "total", "count", "how many", "min", "max",
        "highest", "lowest", "top", "bottom", "compare", "vs", "difference",
        "salary", "department", "performance", "experience", "distribution",
        "correlation", "outlier", "trend", "group", "by",
    ]

    if any(term in lower for term in analytics_terms):
        # Only treat as analytics if numeric columns exist or the user mentioned a column
        numeric_cols = df.select_dtypes(include=[np.number]).columns.tolist()
        mentioned = _mentioned_columns(message, list(df.columns))
        return bool(numeric_cols) or len(mentioned) > 0
